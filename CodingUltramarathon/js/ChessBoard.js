class ChessBoard extends Project {
    constructor(canvas) {
        super(canvas)
    ...
    drawFrame(){
        if(VIDEO!=null){
			...
			chessBoardImage(this.ctx)
			
		} 
    }   
}

function chessBoardImage(ctx){
    const imgData = ctx.getImageData(0,0,CANVAS_SIZE,CANVAS_SIZE)

    for(let y=0; y<=CANVAS_SIZE; y++) {
    	for(let x=0; x<=CANVAS_SIZE; x++) {
    		let index=(y*CANVAS_SIZE+x%CANVAS_SIZE)*4;
			let rowIndex=Math.floor(y/(CANVAS_SIZE/8))
			let colIndex=Math.floor(x/(CANVAS_SIZE/8))
